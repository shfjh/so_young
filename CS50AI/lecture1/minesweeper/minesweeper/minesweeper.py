...
class MinesweeperAI():
    ...
    def conclude(self):
        ...
        for sentence in self.knowledge:
            if sentence.known_mines():
                find_mines += list(sentence.cells)
                new_clue += 1
            if sentence.known_safes():
                find_safes += list(sentence.cells)
                new_clue += 1
        for mine in find_mines:
            self.mark_mine(mine)
        for safe in find_safes:
            self.mark_safe(safe)
        return new_clue
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in [-1, 0 ,1]:
            for j in [-1, 0, 1]:
                if x+i < 0 or x+i >= self.height or y+j < 0 or y+j >= self.width or (x==0 and y==0):
                    continue 
